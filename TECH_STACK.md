...
游刃是一款本地密码管理应用，采用现代化技术栈构建，专注于数据安全性和用户体验。产品以桌面应用形式运行，所有数据存储在本地，确保用户隐私安全。
...
游刃采用现代化技术栈构建，兼顾了安全性、性能和用户体验。通过使用Tauri框架和Rust语言，实现了轻量级、高性能的桌面应用。本地存储设计确保了用户数据的安全性和隐私保护。
